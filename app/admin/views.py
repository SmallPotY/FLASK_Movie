...
def login():
    form = LoginForm()  # 实例表单
    if form.validate_on_submit():  # 如果提交了表单
        data = form.data
...
def pwd():
    ...
    if form.validate_on_submit():
        ...
        from werkzeug.security import generate_password_hash  # 导入哈希解密
...
def tag_list(page=None):
    if page is None:
        page = 1
    page_date = Tag.query.order_by(Tag.addtime.desc()).paginate(page=page, per_page=10)
...
@admin_login_req
def tag_del(id=None):
    tag = Tag.query.filter_by(id=id).first_or_404()  # first_or_404,获取不到记录返回404
...
def movie_list(page=None):
    if page is None:
        page = 1
    page_date = Movie.query.join(Tag).filter(
        Tag.id == Movie.tag_id
    ).order_by(
        Movie.addtime.desc()
...
def preview_list(page=1):
    page_date = Preview.query.order_by(Preview.addtime.desc()).paginate(page=page, per_page=10)
...
def comment_del(id=None):
    ...
    oplog = Oplog(
        admin_id=session['admin_id'],
        ip=request.remote_addr,
        reason="删除收藏：{}".format(comment.content)
    )
    db.session.add(oplog)
    db.session.commit()
...
def moviecol_list(page=1):
    page_date = Moviecol.query.join(Movie).join(User).filter(
        Movie.id == Moviecol.movie_id,
        User.id == Moviecol.user_id
    ).order_by(Moviecol.addtime.desc()).paginate(page=page, per_page=10)

    return render_template("admin/moviecol_list.html", page_date=page_date)
...
def moviecol_del(id=None):
    ...
    oplog = Oplog(
        admin_id=session['admin_id'],
        ip=request.remote_addr,
        reason="删除收藏：{}".format(moviecol.user_id)
    )
    db.session.add(oplog)
    db.session.commit()
...
def oplog_list(page=1):
    page_date = Oplog.query.join(Admin).filter(
        Admin.id == Oplog.admin_id,
    ).order_by(Oplog.addtime.desc()).paginate(page=page, per_page=10)

    return render_template("admin/oplog_list.html", page_date=page_date)
...
def adminloginlog_list(page=1):
    page_date = Adminlog.query.join(Admin).filter(
        Admin.id == Adminlog.admin_id,
    ).order_by(Adminlog.addtime.desc()).paginate(page=page, per_page=10)
    return render_template("admin/adminloginlog_list.html", page_date=page_date)
...
def userloginlog_list(page=1):
    page_date = Userlog.query.join(User).filter(
        User.id == Userlog.user_id,
    ).order_by(Userlog.addtime.desc()).paginate(page=page, per_page=10)

    return render_template("admin/userloginlog_list.html", page_date=page_date)
...
def role_list(page=1):
    page_date = Role.query.order_by(Role.addtime.desc()).paginate(page=page, per_page=10)
    return render_template("admin/role_list.html", page_date=page_date)
...
def role_del(id=None):
    role = Role.query.filter_by(id=id).first_or_404()  # first_or_404,获取不到记录返回404
...
def role_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        oplog = Oplog(
            admin_id=session['admin_id'],
            ip=request.remote_addr,
            reason="编辑角色：{}=>{};{}=>{}".format(old_name, data['name'], old_auths, data['auths'])
        )
        db.session.add(oplog)
        db.session.commit()
        return redirect(url_for('admin.role_list', page=1))
...
def auth_list(page=1):
    page_date = Auth.query.order_by(Auth.addtime.desc()).paginate(page=page, per_page=10)
    return render_template("admin/auth_list.html", page_date=page_date)
...
def auth_del(id=None):
    auth = Auth.query.filter_by(id=id).first_or_404()  # first_or_404,获取不到记录返回404
...
def auth_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        oplog = Oplog(
            admin_id=session['admin_id'],
            ip=request.remote_addr,
            reason="编辑权限：{}=>{};{}=>{}".format(old_name, data['name'], old_url, data['url'])
        )
        db.session.add(oplog)
        db.session.commit()
        return redirect(url_for("admin.auth_list", page=1))
...
def admin_list(page=1):
    page_date = Admin.query.join(Role).filter(
        Role.id == Admin.role_id
    ).order_by(Admin.addtime.desc()).paginate(page=page, per_page=10)
    return render_template("admin/admin_list.html",page_date=page_date)
